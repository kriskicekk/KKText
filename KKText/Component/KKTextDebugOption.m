...
static KKTextDebugOption *_sharedDebugOption = nil;
...
void _sharedDebugSetFunction(const void *value, void *context) {
    id<KKTextDebugTarget> target = (__bridge id<KKTextDebugTarget>)(value);
...
static void _setSharedDebugOption(KKTextDebugOption *option) {
    _initSharedDebug();
...
static void _addDebugTarget(id<KKTextDebugTarget> target) {
    _initSharedDebug();
    pthread_mutex_lock(&_sharedDebugLock);
...
static void _removeDebugTarget(id<KKTextDebugTarget> target) {
    _initSharedDebug();
    pthread_mutex_lock(&_sharedDebugLock);
...
@implementation KKTextDebugOption

- (id)copyWithZone:(NSZone *)zone {
    KKTextDebugOption *op = [self.class new];
...
+ (void)addDebugTarget:(id<KKTextDebugTarget>)target {
    if (target) _addDebugTarget(target);
}

+ (void)removeDebugTarget:(id<KKTextDebugTarget>)target {
    if (target) _removeDebugTarget(target);
}

+ (KKTextDebugOption *)sharedDebugOption {
    return _getSharedDebugOption();
}

+ (void)setSharedDebugOption:(KKTextDebugOption *)option {
