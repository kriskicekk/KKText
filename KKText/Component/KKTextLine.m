...
- (void)reloadBounds {
    ...
    for (NSUInteger r = 0; r < runCount; r++) {
        ...
        KKTextAttachment *attachment = attrs[KKTextAttachmentAttributeName];
        if (attachment) {
            ...
            NSRange runRange = KKTextNSRangeFromCFRange(CTRunGetStringRange(run));
...
- (NSString *)description {
    ...
    [desc appendFormat:@"<KKTextLine: %p> row:%zd range:%tu,%tu",self, self.row, range.location, range.length];
