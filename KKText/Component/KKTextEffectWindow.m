...
@implementation KKTextEffectWindow

+ (instancetype)sharedWindow {
    static KKTextEffectWindow *one = nil;
    if (one == nil) {
        // iOS 9 compatible
    ...
    dispatch_once(&onceToken, ^{
        if (!KKTextIsAppExtension()) {
            one = [self new];
            one.frame = (CGRect){.size = KKTextScreenSize()};
...
- (void)becomeKeyWindow {
    [[KKTextSharedApplication().delegate window] makeKeyWindow];
}

- (UIViewController *)rootViewController {
    for (UIWindow *window in [KKTextSharedApplication() windows]) {
...
- (KKTextDirection)_keyboardDirection {
    CGRect keyboardFrame = [KKTextKeyboardManager defaultManager].keyboardFrame;
    keyboardFrame = [[KKTextKeyboardManager defaultManager] convertRect:keyboardFrame toView:self];
    if (CGRectIsNull(keyboardFrame) || CGRectIsEmpty(keyboardFrame)) return KKTextDirectionNone;
    
    if (CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return KKTextDirectionTop;
    ...
    if (CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame) &&
        ...
        return KKTextDirectionRight;
    
    if (CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame) &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return KKTextDirectionBottom;
    
    if (CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame))
        return KKTextDirectionLeft;
    
    return KKTextDirectionNone;
}
...
- (CGPoint)_correctedCenter:(CGPoint)center forMagnifier:(KKTextMagnifier *)mag rotation:(CGFloat)rotation {
    CGFloat degree = KKTextRadiansToDegrees(rotation);
    ...
    if (degree <= 1 || degree >= 7) { //top
        if (mag.type == KKTextMagnifierTypeCaret) {
            if (center.y < caretExt)
                center.y = caretExt;
        } else if (mag.type == KKTextMagnifierTypeRanged) {
 ...
 */
- (CGFloat)_updateMagnifier:(KKTextMagnifier *)mag {
    UIApplication *app = KKTextSharedApplication();
    if (!app) return 0;
    ...
    CGPoint captureCenter = [self kk_convertPoint:mag.hostCaptureCenter fromViewOrWindow:hostView];
    ...
    CGAffineTransform trans = KKTextCGAffineTransformGetFromViews(hostView, self);
    CGFloat rotation = KKTextCGAffineTransformGetRotation(trans);
    ...
    for (UIWindow *window in windows) {
        ...
        CGContextConcatCTM(context, KKTextCGAffineTransformGetFromViews(window, self));
...
- (void)showMagnifier:(KKTextMagnifier *)mag {
    ...
    CGPoint center = [self kk_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    CGAffineTransform trans = CGAffineTransformMakeRotation(rotation);
    trans = CGAffineTransformScale(trans, 0.3, 0.3);
    mag.transform = trans;
    mag.center = center;
    if (mag.type == KKTextMagnifierTypeRanged) {
        mag.alpha = 0;
    }
    NSTimeInterval time = mag.type == KKTextMagnifierTypeCaret ? 0.08 : 0.1;
...
- (void)hideMagnifier:(KKTextMagnifier *)mag {
    ...
    CGFloat rotation = [self _updateMagnifier:mag];
    CGPoint center = [self kk_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    NSTimeInterval time = mag.type == KKTextMagnifierTypeCaret ? 0.20 : 0.15;
    [UIView animateWithDuration:time delay:0 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState animations:^{
...
- (void)_updateSelectionGrabberDot:(KKSelectionGrabberDot *)dot selection:(KKTextSelectionView *)selection{
    dot.mirror.hidden = YES;
    if (selection.hostView.clipsToBounds == YES && dot.kk_visibleAlpha > 0.1) {
        CGRect dotRect = [dot kk_convertRect:dot.bounds toViewOrWindow:self];
        ...
        if (!dotInKeyboard) {
            ...
            if (KKTextCGRectGetArea(intersection) < KKTextCGRectGetArea(dotRect)) {
                CGFloat dist = KKTextCGPointGetDistanceToRect(KKTextCGRectGetCenter(dotRect), hostRect);
    ...
    CGPoint center = [dot kk_convertPoint:CGPointMake(CGRectGetWidth(dot.frame) / 2, CGRectGetHeight(dot.frame) / 2) toViewOrWindow:self];
