...
@protocol KKTextLinePositionModifier;
 ...
 */
extern const CGSize KKTextContainerMaxSize;

/**
 The KKTextContainer class defines a region in which text is laid out.
 KKTextLayout class uses one or more KKTextContainer objects to generate layouts.
 
 A KKTextContainer defines rectangular regions (`size` and `insets`) or 
 ...
 Example:
     ...
     └─────────────────────────────┘
 */
@interface KKTextContainer : NSObject <NSCoding, NSCopying>
...
/// The constrained size. (if the size is larger than KKTextContainerMaxSize, it will be clipped)
...
@property (nullable, copy) id<KKTextLinePositionModifier> linePositionModifier;
@end


/**
 The KKTextLinePositionModifier protocol declares the required method to modify
 the line position in text layout progress. See `KKTextLinePositionSimpleModifier` for example.
 */
@protocol KKTextLinePositionModifier <NSObject, NSCopying>
@required
/**
 This method will called before layout is completed. The method should be thread-safe.
 @param lines     An array of KKTextLine.
 @param text      The full text.
 @param container The layout container.
 */
- (void)modifyLines:(NSArray<KKTextLine *> *)lines fromText:(NSAttributedString *)text inContainer:(KKTextContainer *)container;
@end


/**
 A simple implementation of `KKTextLinePositionModifier`. It can fix each line's position
 to a specified value, lets each line of height be the same.
 */
@interface KKTextLinePositionSimpleModifier : NSObject <KKTextLinePositionModifier>
...
/**
 KKTextLayout class is a readonly class stores text layout result.
 ...
 example: (layout with a circle exclusion path)
     ...
     └──────────────────────────┘
 */
@interface KKTextLayout : NSObject <NSCoding>
...
*/
+ (nullable KKTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text;
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable KKTextLayout *)layoutWithContainer:(KKTextContainer *)container text:(NSAttributedString *)text;
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable KKTextLayout *)layoutWithContainer:(KKTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range;

/**
 Generate layouts with the given containers and text.
 
 @param containers An array of KKTextContainer object (if nil, returns nil).
 @param text       The text (if nil, returns nil).
 @return An array of KKTextLayout object (the count is same as containers),
    or nil when an error occurs.
 */
+ (nullable NSArray<KKTextLayout *> *)layoutWithContainers:(NSArray<KKTextContainer *> *)containers
...
///< The text container
@property (nonatomic, strong, readonly) KKTextContainer *container;
///< The full text
...
///< Array of `KKTextLine`, no truncated
@property (nonatomic, strong, readonly) NSArray<KKTextLine *> *lines;
///< KKTextLine with truncated token, or nil
@property (nullable, nonatomic, strong, readonly) KKTextLine *truncatedLine;
///< Array of `KKTextAttachment`
@property (nullable, nonatomic, strong, readonly) NSArray<KKTextAttachment *> *attachments;
 ...
 */
- (nullable KKTextPosition *)closestPositionToPoint:(CGPoint)point;
...
/**
 ...
 @return A text position, or nil if not found.
 */
- (nullable KKTextPosition *)positionForPoint:(CGPoint)point
 ...
 */
- (nullable KKTextRange *)textRangeAtPoint:(CGPoint)point;
...
/**
 ...
 at point. Or nil if not found.
 */
- (nullable KKTextRange *)closestTextRangeAtPoint:(CGPoint)point;
 ...
 */
- (nullable KKTextRange *)textRangeByExtendingPosition:(KKTextPosition *)position;
...
/**
 ...
 @return An array of `KKTextSelectionRect` objects that encompass the selection.
 If not found, the array is empty.
 */
- (NSArray<KKTextSelectionRect *> *)selectionRectsForRange:(KKTextRange *)range;
...
/**
 ...
 If not found, the array is empty.
 */
- (NSArray<KKTextSelectionRect *> *)selectionRectsWithoutStartAndEndForRange:(KKTextRange *)range;
...
/**
 ...
 @param range An object representing a range in text.
 @return An array of `KKTextSelectionRect` objects contains the start and end to
 the selection. If not found, the array is empty.
 */
- (NSArray<KKTextSelectionRect *> *)selectionRectsWithOnlyStartAndEndForRange:(KKTextRange *)range;
...
- (void)drawInContext:(nullable CGContextRef)context
                ...
                debug:(nullable KKTextDebugOption *)debug
