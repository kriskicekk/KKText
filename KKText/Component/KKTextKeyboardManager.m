...
static int _KKTextKeyboardViewFrameObserverKey;

/// Observer for view's frame/bounds/center/transform
@interface _KKTextKeyboardViewFrameObserver : NSObject
...
@implementation _KKTextKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_KKTextKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_KKTextKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_KKTextKeyboardViewFrameObserverKey);
}
...
@implementation KKTextKeyboardManager {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"KKTextKeyboardManager init error" reason:@"Use 'defaultManager' to get instance." userInfo:nil];
    return [super init];
}
...
- (void)_initFrameObserver {
    ...
    _KKTextKeyboardViewFrameObserver *observer = [_KKTextKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_KKTextKeyboardViewFrameObserver new];
...
+ (instancetype)defaultManager {
    static KKTextKeyboardManager *mgr = nil;
...
- (void)addObserver:(id<KKTextKeyboardObserver>)observer {
    if (!observer) return;
    [_observers addObject:observer];
}

- (void)removeObserver:(id<KKTextKeyboardObserver>)observer {
    if (!observer) return;
...
- (void)_notifyAllObservers {
    ...
    KKTextKeyboardTransition trans = {0};
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<KKTextKeyboardObserver> observer in _observers.copy) {
