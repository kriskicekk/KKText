...
@interface _KKTextMagnifierCaret : KKTextMagnifier {
    UIImageView *_contentView;
    UIImageView *_coverView;
}
@end

@implementation _KKTextMagnifierCaret
...
- (KKTextMagnifierType)type {
    return KKTextMagnifierTypeCaret;
}
...
+ (UIImage *)coverImage {
    ...
    dispatch_once(&onceToken, ^{
        ...
        image = [renderer imageWithActions:^(UIGraphicsImageRendererContext *rendererContext) {
            ...
            CGPathRef strokePath = CGPathCreateWithEllipseInRect(KKTextCGRectPixelHalf(rect), NULL);
            ...
            CGContextSaveGState(context); {
                ...
                CGContextSetLineWidth(context, KKTextCGFloatFromPixel(1));
...
@interface _KKTextMagnifierRanged : KKTextMagnifier {
    UIImageView *_contentView;
    UIImageView *_coverView;
}
@end
...
@implementation _KKTextMagnifierRanged
#define kMultiple 1.2
#define kSize CGSizeMake(141, 60)
#define kPadding KKTextCGFloatPixelHalf(6.0)
#define kRadius 6.0
#define kHeight 32.0
#define kArrow 14.0
...
- (KKTextMagnifierType)type {
    return KKTextMagnifierTypeRanged;
}
...
+ (UIImage *)coverImage {
    ...
    dispatch_once(&onceToken, ^{
        ...
        image = [renderer imageWithActions:^(UIGraphicsImageRendererContext *rendererContext) {
            ...
            CGPathMoveToPoint(arrowPath, NULL, size.width / 2 - kArrow, KKTextCGFloatPixelFloor(kPadding) + kHeight);
            CGPathAddLineToPoint(arrowPath, NULL, size.width / 2 + kArrow, KKTextCGFloatPixelFloor(kPadding) + kHeight);
...
@implementation KKTextMagnifier

+ (id)magnifierWithType:(KKTextMagnifierType)type {
    switch (type) {
        case KKTextMagnifierTypeCaret :return [_KKTextMagnifierCaret new];
        case KKTextMagnifierTypeRanged :return [_KKTextMagnifierRanged new];
    }
    return nil;
}
...
- (id)initWithFrame:(CGRect)frame {
    // class cluster
    if ([self isMemberOfClass:[KKTextMagnifier class]]) {
