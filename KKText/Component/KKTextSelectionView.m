...
@implementation KKSelectionGrabberDot
...
@implementation KKSelectionGrabber

- (instancetype) initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (!self) return nil;
    _dot = [[KKSelectionGrabberDot alloc] initWithFrame:CGRectMake(0, 0, 10, 10)];
    return self;
}

- (void)setDotDirection:(KKTextDirection)dotDirection {
    _dotDirection = dotDirection;
    [self addSubview:_dot];
    CGRect frame = _dot.frame;
    CGFloat ofs = 0.5;
    if (dotDirection == KKTextDirectionTop) {
        frame.origin.y = -frame.size.height + ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == KKTextDirectionRight) {
        frame.origin.x = self.bounds.size.width - ofs;
        frame.origin.y = (self.bounds.size.height - frame.size.height) / 2;
    } else if (dotDirection == KKTextDirectionBottom) {
        frame.origin.y = self.bounds.size.height - ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == KKTextDirectionLeft) {
...
- (CGRect)touchRect {
    ...
    if (_dotDirection == KKTextDirectionTop) {
        insets.top = -kTouchDotExtend;
    } else if (_dotDirection == KKTextDirectionRight) {
        insets.right = -kTouchDotExtend;
    } else if (_dotDirection == KKTextDirectionBottom) {
        insets.bottom = -kTouchDotExtend;
    } else if (_dotDirection == KKTextDirectionLeft) {
...
@interface KKTextSelectionView ()
@property (nonatomic, strong) NSTimer *caretTimer;
@property (nonatomic, strong) UIView *caretView;
@property (nonatomic, strong) KKSelectionGrabber *startGrabber;
@property (nonatomic, strong) KKSelectionGrabber *endGrabber;
@property (nonatomic, strong) NSMutableArray *markViews;
@end

@implementation KKTextSelectionView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _startGrabber = [KKSelectionGrabber new];
    _startGrabber.dotDirection = KKTextDirectionTop;
    _startGrabber.hidden = YES;
    _endGrabber = [KKSelectionGrabber new];
    _endGrabber.dotDirection = KKTextDirectionBottom;
...
- (void)_startBlinks {
    [_caretTimer invalidate];
    if (_caretVisible) {
        _caretTimer = [NSTimer timerWithTimeInterval:kBlinkDuration target:[KKTextWeakProxy proxyWithTarget:self] selector:@selector(_doBlink) userInfo:nil repeats:YES];
...
- (void)setVerticalForm:(BOOL)verticalForm {
    if (_verticalForm != verticalForm) {
        ...
        self.startGrabber.dotDirection = verticalForm ? KKTextDirectionRight : KKTextDirectionTop;
        self.endGrabber.dotDirection = verticalForm ? KKTextDirectionLeft : KKTextDirectionBottom;
    }
}
...
- (CGRect)_standardCaretRect:(CGRect)caretRect {
    ...
    caretRect = KKTextCGRectPixelRound(caretRect);
...
- (void)setSelectionRects:(NSArray *)selectionRects {
    ...
    [selectionRects enumerateObjectsUsingBlock: ^(KKTextSelectionRect *r, NSUInteger idx, BOOL *stop) {
        CGRect rect = r.rect;
...
- (BOOL)isStartGrabberContainsPoint:(CGPoint)point {
    ...
    if (CGRectIntersectsRect(startRect, endRect)) {
        CGFloat distStart = KKTextCGPointGetDistanceToPoint(point, KKTextCGRectGetCenter(startRect));
        CGFloat distEnd = KKTextCGPointGetDistanceToPoint(point, KKTextCGRectGetCenter(endRect));
...
- (BOOL)isSelectionRectsContainsPoint:(CGPoint)point {
    ...
    for (KKTextSelectionRect *rect in _selectionRects) {
