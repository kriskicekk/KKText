...
@interface KKLabel() <KKTextDebugTarget, KKTextAsyncLayerDelegate> {
    NSMutableAttributedString *_innerText; ///< nonnull
    KKTextLayout *_innerLayout;
    KKTextContainer *_innerContainer; ///< nonnull
    ...
    KKTextLayout *_shrinkInnerLayout;
    KKTextLayout *_shrinkHighlightLayout;
...
- (void)_updateLayout {
    _innerLayout = [KKTextLayout layoutWithContainer:_innerContainer text:_innerText];
    _shrinkInnerLayout = [KKLabel _shrinkLayoutWithLayout:_innerLayout];
}
...
- (void)_clearInnerLayout {
    ...
    KKTextLayout *layout = _innerLayout;
    _innerLayout = nil;
    _shrinkInnerLayout = nil;
    dispatch_async(KKLabelGetReleaseQueue(), ^{
...
- (KKTextLayout *)_highlightLayout {
    return _shrinkHighlightLayout ? _shrinkHighlightLayout : _highlightLayout;
}

+ (KKTextLayout *)_shrinkLayoutWithLayout:(KKTextLayout *)layout {
    if (layout.text.length && layout.lines.count == 0) {
        KKTextContainer *container = layout.container.copy;
        container.maximumNumberOfRows = 1;
        CGSize containerSize = container.size;
        if (!container.verticalForm) {
            containerSize.height = KKTextContainerMaxSize.height;
        } else {
            containerSize.width = KKTextContainerMaxSize.width;
        }
        container.size = containerSize;
        return [KKTextLayout layoutWithContainer:container text:layout.text];
    } else {
        return nil;
    }
}
...
- (void)_trackDidLongPress {
    ...
    if (_state.hasLongPressAction && _textLongPressAction) {
        ...
        KKTextRange *textRange = [self._innerLayout textRangeAtPoint:point];
    ...
    if (_highlight) {
        KKTextAction longPressAction = _highlight.longPressAction ? _highlight.longPressAction : _highlightLongPressAction;
        if (longPressAction) {
            KKTextPosition *start = [KKTextPosition positionWithOffset:_highlightRange.location];
            KKTextPosition *end = [KKTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:KKTextAffinityBackward];
            KKTextRange *range = [KKTextRange rangeWithStart:start end:end];
...
- (KKTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    ...
    KKTextHighlight *highlight = [_innerText attribute:KKTextHighlightAttributeName
                                               atIndex:startIndex
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _shrinkHighlightLayout = [KKLabel _shrinkLayoutWithLayout:_highlightLayout];
...
- (void)_updateOuterLineBreakMode {
    if (_innerContainer.truncationType) {
        switch (_innerContainer.truncationType) {
            ...
            case KKTextTruncationTypeEnd: {
                _lineBreakMode = NSLineBreakByTruncatingTail;
            } break;
            case KKTextTruncationTypeMiddle: {
                _lineBreakMode = NSLineBreakByTruncatingMiddle;
            } break;
            default:break;
        }
    } else {
        _lineBreakMode = _innerText.kk_lineBreakMode;
    }
}

- (void)_updateOuterTextProperties {
    _text = [_innerText kk_plainTextForRange:NSMakeRange(0, _innerText.length)];
    _font = _innerText.kk_font;
    if (!_font) _font = [self _defaultFont];
    _textColor = _innerText.kk_color;
    if (!_textColor) _textColor = [UIColor blackColor];
    _textAlignment = _innerText.kk_alignment;
    _lineBreakMode = _innerText.kk_lineBreakMode;
    NSShadow *shadow = _innerText.kk_shadow;
...
- (void)_initLabel {
    ...
    _debugOption = [KKTextDebugOption sharedDebugOption];
    [KKTextDebugOption addDebugTarget:self];
    
    _font = [self _defaultFont];
    _textColor = [UIColor blackColor];
    _textVerticalAlignment = KKTextVerticalAlignmentCenter;
    _numberOfLines = 1;
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    if ((!_verticalForm && size.width == self.bounds.size.width) ||
        ...
        KKTextLayout *layout = self._innerLayout;
        BOOL contains = NO;
...
- (NSString *)accessibilityLabel {
    return [_innerLayout.text kk_plainTextForRange:_innerLayout.text.kk_rangeOfAll];
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    [self _initLabel];
    KKTextContainer *innerContainer = [aDecoder decodeObjectForKey:@"innerContainer"];
    if (innerContainer) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_state.touchMoved && _highlight) {
            KKTextHighlight *highlight = [self _getHighlightAtPoint:point range:NULL];
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            if (!_state.touchMoved || [self _getHighlightAtPoint:point range:NULL] == _highlight) {
                KKTextAction tapAction = _highlight.tapAction ? _highlight.tapAction : _highlightTapAction;
                if (tapAction) {
                    KKTextPosition *start = [KKTextPosition positionWithOffset:_highlightRange.location];
                    KKTextPosition *end = [KKTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:KKTextAffinityBackward];
                    KKTextRange *range = [KKTextRange rangeWithStart:start end:end];
...
- (void)setText:(NSString *)text {
    ...
    [_innerText kk_removeDiscontinuousAttributesInRange:NSMakeRange(0, _innerText.length)];
    if (needAddAttributes) {
        _innerText.kk_font = _font;
...
- (void)setTextLayout:(KKTextLayout *)textLayout {
...
- (void)setDisplaysAsynchronously:(BOOL)displaysAsynchronously {
    ...
    ((KKTextAsyncLayer *)self.layer).displaysAsynchronously = displaysAsynchronously;
}
...
- (CGSize)intrinsicContentSize {
    if (_preferredMaxLayoutWidth == 0) {
        KKTextContainer *container = [_innerContainer copy];
        container.size = KKTextContainerMaxSize;
...
#pragma mark - KKTextAsyncLayerDelegate

- (KKTextAsyncLayerDisplayTask *)newAsyncDisplayTask {
    
    // capture current context
    BOOL contentsNeedFade = _state.contentsNeedFade;
    NSAttributedString *text = _innerText;
    KKTextContainer *container = _innerContainer;
    KKTextVerticalAlignment verticalAlignment = _textVerticalAlignment;
    KKTextDebugOption *debug = _debugOption;
    ...
    __block KKTextLayout *layout = (_state.showingHighlight && _highlightLayout) ? self._highlightLayout : self._innerLayout;
    __block KKTextLayout *shrinkLayout = nil;
    ...
    // create display task
    KKTextAsyncLayerDisplayTask *task = [KKTextAsyncLayerDisplayTask new];
    ...
    task.display = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        ...
        KKTextLayout *drawLayout = layout;
        if (layoutNeedUpdate) {
            layout = [KKTextLayout layoutWithContainer:container text:text];
            shrinkLayout = [KKLabel _shrinkLayoutWithLayout:layout];
            if (isCancelled()) return;
            layoutUpdated = YES;
        ...
        if (verticalAlignment == KKTextVerticalAlignmentCenter) {
        ...
        } else if (verticalAlignment == KKTextVerticalAlignmentBottom) {
            if (drawLayout.container.isVerticalForm) {
        ...
        point = KKTextCGPointPixelRound(point);
    ...
    task.didDisplay = ^(CALayer *layer, BOOL finished) {
        ...
        __strong KKLabel *view = (KKLabel *)layer.delegate;
        if (!view) return;
...
@implementation KKLabel (IBInspectableProperties)
