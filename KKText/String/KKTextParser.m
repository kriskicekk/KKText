...
@implementation KKTextSimpleMarkdownParser {
    UIFont *_font;
...
- (void)_updateFonts {
    ...
    _boldFont = KKTextFontWithBold(_font);
    _italicFont = KKTextFontWithItalic(_font);
    _boldItalicFont = KKTextFontWithBoldItalic(_font);
...
- (void)setColorWithBrightTheme {
    ...
    _border = [KKTextBorder new];
    _border.lineStyle = KKTextLineStyleSingle;
    ...
    _border.strokeWidth = KKTextCGFloatFromPixel(1);
}
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    [text kk_removeAttributesInRange:NSMakeRange(0, text.length)];
    text.kk_font = _font;
    text.kk_color = _textColor;
    ...
    [_regexHeader enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location, whiteLen + sharpLen)];
        [text kk_setColor:_headerTextColor range:NSMakeRange(r.location + whiteLen + sharpLen, r.length - whiteLen - sharpLen)];
        [text kk_setFont:_headerFonts[sharpLen - 1] range:result.range];
    }];
    ...
    [_regexH1 enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (linebreak.location != NSNotFound) {
            [text kk_setColor:_headerTextColor range:NSMakeRange(r.location, linebreak.location - r.location)];
            [text kk_setFont:_headerFonts[0] range:NSMakeRange(r.location, linebreak.location - r.location + 1)];
            [text kk_setColor:_controlTextColor range:NSMakeRange(linebreak.location + linebreak.length, r.location + r.length - linebreak.location - linebreak.length)];
        }
    }];
    ...
    [_regexH2 enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (linebreak.location != NSNotFound) {
            [text kk_setColor:_headerTextColor range:NSMakeRange(r.location, linebreak.location - r.location)];
            [text kk_setFont:_headerFonts[1] range:NSMakeRange(r.location, linebreak.location - r.location + 1)];
            [text kk_setColor:_controlTextColor range:NSMakeRange(linebreak.location + linebreak.length, r.location + r.length - linebreak.location - linebreak.length)];
        }
    }];
    ...
    [_regexBreakline enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        [text kk_setColor:_controlTextColor range:result.range];
    }];
    ...
    [_regexEmphasis enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location, 1)];
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 1, 1)];
        [text kk_setFont:_italicFont range:NSMakeRange(r.location + 1, r.length - 2)];
    }];
    ...
    [_regexStrong enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text kk_setFont:_boldFont range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexStrongEmphasis enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location, 3)];
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 3, 3)];
        [text kk_setFont:_boldItalicFont range:NSMakeRange(r.location + 3, r.length - 6)];
    }];
    ...
    [_regexUnderline enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text kk_setTextUnderline:[KKTextDecoration decorationWithStyle:KKTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexStrikethrough enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text kk_setTextStrikethrough:[KKTextDecoration decorationWithStyle:KKTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexInlineCode enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location, len)];
        [text kk_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - len, len)];
        [text kk_setColor:_inlineTextColor range:NSMakeRange(r.location + len, r.length - 2 * len)];
        [text kk_setFont:_monospaceFont range:r];
        [text kk_setTextBorder:_border.copy range:r];
    }];
    ...
    [_regexLink enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text kk_setColor:_linkTextColor range:r];
    }];
    ...
    [_regexLinkRefer enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text kk_setColor:_controlTextColor range:r];
    }];
    ...
    [_regexCodeBlock enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (lenStart + lenEnd < r.length) {
            ...
            [text kk_setColor:_codeTextColor range:codeR];
            [text kk_setFont:_monospaceFont range:codeR];
            KKTextBorder *border = [KKTextBorder new];
            border.lineStyle = KKTextLineStyleSingle;
            ...
            border.strokeWidth = KKTextCGFloatFromPixel(2);
            [text kk_setTextBlockBorder:_border.copy range:codeR];
        }
    }];
...
@implementation KKTextSimpleEmoticonParser {
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    for (NSUInteger i = 0, max = matches.count; i < max; i++) {
        ...
        CTFontRef font = (__bridge CTFontRef)([text kk_attribute:NSFontAttributeName atIndex:oneRange.location]);
        if (font) fontSize = CTFontGetSize(font);
        NSMutableAttributedString *atr = [NSAttributedString kk_attachmentStringWithEmojiImage:emoticon fontSize:fontSize];
        [atr kk_setTextBackedString:[KKTextBackedString stringWithString:subStr] range:NSMakeRange(0, atr.length)];
        [text replaceCharactersInRange:oneRange withString:atr.string];
        [text kk_removeDiscontinuousAttributesInRange:NSMakeRange(oneRange.location, atr.length)];
        [text addAttributes:atr.kk_attributes range:NSMakeRange(oneRange.location, atr.length)];
